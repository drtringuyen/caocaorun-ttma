...
class DrawPoop {
	...
	public DrawPoop(TouchScreen tscreen,Cell[][] maze, ScreenProperties screenProperties) {
		...
		this.poop_maze_y=0;
		this.poop_maze_x=0;
		this.poop=Bitmap.createScaledBitmap(poop, SCALED_SIZE, SCALED_SIZE, false);
	...
	private void update(float x_end, float x_start,float y_end, float y_start){
		...
		int goTo = controlPoop.goTo(x_end, x_start, y_end, y_start,currentCell);
		if(checkPoopAtBegin() && goTo==0) goTo=-1;
		if(checkPoopAtEnd() && goTo==2) goTo=-1;
